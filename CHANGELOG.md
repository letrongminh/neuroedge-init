...
| `nightly-hardware.yml` | 01:00 UTC+7 hằng đêm · chạy tay | `firmware-build` (ESP-IDF 5.2.1, ngân sách flash Q-3) · `upstream-drift` (bản mới nhất được phép so với lock, chạy test; không bao giờ đỏ) · `drift-issue` (mở, cập nhật hoặc đóng **một** issue nhãn `dependency-drift` — Q-32) · `memory-spike` · `report` |
| `security.yml` | Mỗi PR và push lên `main` · hằng tuần · chạy tay | `pip-audit` (mọi pin trong `requirements-lock.txt`; lỗ hổng đã biết ⇒ đỏ, ngoại lệ chỉ qua `python/pip-audit-ignore.txt` có lý do) · `gitleaks` (toàn lịch sử mọi nhánh và tag; ngoài PR thêm head của mọi PR) · `codeql` (Python, GitHub Actions) · `codeql-c` (firmware, build trong `espressif/idf:v5.4`) |
...
(`.github/dependabot.yml`) đề xuất bản mới hằng tuần. `actionlint` sạch
