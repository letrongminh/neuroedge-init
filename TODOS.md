...
| 20 | **Bề mặt tương tác thật cho `escalate`/`ask`** (người nhận, UI hỏi lại). Q-17 đặc tả v1.0 là chặn + vết ghi + hook no-op. *Một phần đã chốt (Q-26): ai xác nhận `ask` — chỉ người qua kênh thiết bị; vòng xác nhận trên `sim` là TSK-S3-26. Còn hoãn: người nhận `escalate`* | Cần FSM thoại (`TSK-S2-07`) và sim web UI (`TSK-S2-09`), cả hai hoãn | **Sprint 5**, hoặc khách đầu tiên cần escalation tới người thật |
