...
| 4 | **Xử lý PII trong vết ghi.** Vết ghi chứa intent, tức nội dung người dùng nói | Chưa có người dùng thật; `record --anonymize` băm chữ thô tại nguồn nhưng không phải mặc định | **Người dùng bên ngoài đầu tiên** (tức ngay sau khi `TSK-S3-14` publish thật) |
| 6 | **Metric · dashboard · alerting** | Không có dịch vụ nào để alert trong Giai đoạn 1. Vết ghi **là** nền quan sát, và `gate explain` trả lời được câu "vì sao lần đó bị chặn" | Fleet OS |
...
| 13 | **`neuroedge board check <file>` báo đạt/thiếu so với bậc**, chạy bộ vector tuân thủ trên một profile bậc 3 (`E4`) | Bộ vector tuân thủ chưa tồn tại. Kiểm tĩnh hôm nay chỉ có cho profile trong `boards/` (`board.v1.json`, `tests/test_boards.py`, `board show`); CLI chưa nhận một tệp profile bất kỳ | **Khối P1** (bộ công cụ port cộng đồng) mở, hoặc bản port bậc 3 đầu tiên được gửi tới |
...
| 41 | **Repo công khai chứa gì** (Q-39). Công khai là increment I6, khi demo thoại chạy trên `sim`, `linux` và Box-3. Hai hướng đã nêu: công khai nguyên repo sau khi quét gitleaks toàn lịch sử, hoặc repo mới với lịch sử mới chỉ gồm mã, lược đồ, đặc tả và tài liệu người dùng (PRD, proposal, `docs/business/`, `docs/archive/` giữ ở repo riêng tư) | CPO chọn hoãn tới lúc có demo; chưa có demo thì chưa cần quyết | **I6 mở**: CPO chọn một hướng; nếu tách repo thì tách CONTRIBUTING, CLAUDE.md và các test đang dẫn PRD/roadmap trước khi fork |
