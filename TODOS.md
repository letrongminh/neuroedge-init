...
| 14 | **Bất biến `sim` không giàu hơn bo mạch tham chiếu (#7) khi có `vision.in`.** `sim-default` sao Box-3 nên không được khai camera, mà Box-3 bị cấm khai `vision_in` (RFC §9.2) → agent thị giác không chạy được trên target chính thức, Action CI cho khung hình không có đường `sim` (`R6`) | Chưa có agent thị giác nào; cách sửa (mỗi profile `sim` sao đúng một bo mạch tham chiếu, ví dụ `sim-vision` ↔ `linux` có camera) cần bo mạch tham chiếu thị giác | **TSK-V1b-02** (camera ảo trong `sim`) bắt đầu |
