...
| **FR-CLI-10** | Tích hợp tool | `neuroedge mcp tools [--json\|--openai] [--external]` · `neuroedge mcp serve [--agent a.toml] [--ui]` | P1 | `tools` in schema mỗi `@action`; `serve` là máy chủ MCP qua stdio (`--ui`: cùng phiên với giao diện web `sim`), mỗi `tools/call` đi qua `dispatch()` và gate (Q-24); thiếu extra `mcp` ⇒ mã 1 kèm cách cài | §4.8 |
