...
| **Q-4** | Nhà cung cấp System 1/2 ở v1.0 | **ĐÃ CHỐT** *(sửa 2026-09-23)* | **System 1:** Jev (cloud) + **fallback cục bộ = bộ nhận diện lệnh cố định** theo Q-14 (thay Sherpa-ONNX intent extractor); **System 2:** **`claude-sonnet-5`** & GPT-4o-mini, **qua LiteLLM** (Q-10). Kết nối qua lớp trừu tượng provider OSS (Q-12): System 2 theo chuẩn OpenAI; Jev không phải model chat nên nối qua System One API (`POST {api_base}/systemone`, `typesafe/jev-1.13` trên OpenRouter) bằng adapter dựng sẵn, bảng `[system_one]` (TSK-I4-02). |
