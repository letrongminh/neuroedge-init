...
| **FR-MDL-11** | **Vòng tool của System 2:** kết quả mỗi tool call (kể cả BLOCK và lý do) trả lại cho System 2 để trả lời người dùng; số vòng có giới hạn khai báo | P0 *(cùng TSK-S2-11)* | Gate chặn ⇒ System 2 nói lý do thay vì lặp lại lời gọi; quá giới hạn vòng ⇒ dừng, ghi sự kiện | `docs/spec/tool_calling.md` §4 |
...
| **FR-ACE-08** | **Ràng buộc tham số trong gate (Q-25):** gate khai khoảng/tập giá trị cho tham số của hành động; kế thừa chỉ thu hẹp | P1 *(chờ RFC-0005)* | Tool call với tham số ngoài khoảng ⇒ BLOCK `argument_out_of_range`, chân không đổi; gate con nới khoảng ⇒ `GateInheritanceError` | RFC-0005 |
...
| **FR-ACE-10** | **Chỉ người xác nhận `ask` (Q-26):** lời xác nhận đến từ kênh thiết bị, gắn với lần bị chặn, dùng một lần, có hạn | P0 | Xác nhận từ `system_two` hoặc `mcp` bị từ chối; xác nhận quá hạn hoặc dùng lại ⇒ không có tác dụng; xác nhận hợp lệ ⇒ gate lượng giá lại | `docs/spec/tool_calling.md` §6 |
...
| **FR-CLI-10** | Tích hợp tool | `neuroedge mcp tools [--json\|--openai]` · `neuroedge mcp serve [--agent a.toml] [--ui]` | P1 | `tools` in schema mỗi `@action`; `serve` là máy chủ MCP qua stdio (`--ui`: cùng phiên với giao diện web `sim`), mỗi `tools/call` đi qua `dispatch()` và gate (Q-24); thiếu extra `mcp` ⇒ mã 1 kèm cách cài | §4.8 |
...
| **Q-25** | Ràng buộc tham số của tool call | **ĐÃ CHỐT hướng** *(2026-09-23; chi tiết ở RFC-0005, đang thảo luận)* | Ràng buộc tham số (khoảng, tập giá trị, độ dài) nằm **trong gate**, không trong `agent.toml`: đi cùng gate khi chia sẻ qua registry, kế thừa chỉ thu hẹp, `gate lint` chứng minh được, và đi vào `inputSchema` để mô hình thấy trước. Làm **trước** walker C (TSK-S4-02) để bố cục nhị phân Q-23 có nút tham số ngay từ bản đầu. Bác bỏ `argument_facts` ở agent: tác giả agent nới lỏng được mà lint không thấy. |
| **Q-26** | Ai xác nhận `on_block: ask` | **ĐÃ CHỐT** *(2026-09-23)* | Chỉ **người, qua kênh thiết bị** (giọng nói/chữ khớp ngữ pháp, nút UI). Nguồn `system_two` và `mcp` không phát được lời xác nhận. Xác nhận gắn `call_id` + `gate_digest`, dùng một lần, có TTL, và làm gate **lượng giá lại** với `human_confirmed = true` — không bỏ qua gate. Đóng một phần `TODOS.md` #20. `docs/spec/tool_calling.md` §6 |
