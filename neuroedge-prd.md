...
| **FR-MDL-10** | **Hành động là tool call (Q-24):** mọi `@action` là một tool có schema sinh từ chữ ký; `SystemOne`, `SystemTwo`, ngữ pháp lệnh cục bộ và client MCP gọi hành động bằng cùng một `ToolCall`, qua cùng một gate | P0 | Tool lạ hoặc tham số lạ/sai kiểu ⇒ `REJECTED`, không chân nào đổi; cùng một lời gọi từ nguồn khác nhau nhận cùng phán quyết trừ khi gate đọc `call_source`; mất mạng thì câu khớp ngữ pháp vẫn thành tool call | §3.6, Q-24 |
...
| **FR-CLI-10** | Tích hợp tool | `neuroedge mcp tools [--json\|--openai]` · `neuroedge mcp serve [--agent a.toml]` | P1 | `tools` in schema mỗi `@action`; `serve` là máy chủ MCP qua stdio, mỗi `tools/call` đi qua `dispatch()` và gate (Q-24); thiếu extra `mcp` ⇒ mã 1 kèm cách cài | §4.8 |
