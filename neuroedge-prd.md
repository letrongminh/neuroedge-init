...
| **Q-21** | Mô phỏng theo tầng bằng OSS đã kiểm chứng | **ĐỀ XUẤT** *(2026-09-23, chờ kỹ thuật trưởng duyệt)* | Không tự viết emulator. Mỗi tầng kiểm thử một công cụ mở: `SimHAL` (logic, mỗi commit) · `gpio-sim` (GPIO `linux`, mỗi PR — đang dùng) · `snd-aloop`/PulseAudio null-sink (âm thanh `linux`) · khung hình trong bộ nhớ + digest (`display`) · **mã C thuần của firmware biên dịch trên host** và chạy bảng sự thật mỗi PR (TSK-S4-07) · **Espressif QEMU** cho boot/logic `esp32s3` hằng đêm (TSK-S4-08) · bo mạch thật cho âm thanh, màn hình, bộ nhớ. **Không dùng:** Renode (không có ESP32-S3), trình mô phỏng Wokwi (mã đóng, token, không I2S), `iio_simple_dummy`. Lý do: kiểm được walker C trước khi bo mạch về, không thêm phụ thuộc vào lõi MIT (công cụ chạy riêng, Phụ lục H.4 của proposal). Không thay được spike bộ nhớ TSK-S1-10 (QEMU không giả lập I2S/AFE). Chi tiết: proposal §3.2 *Mô phỏng theo tầng*. |
