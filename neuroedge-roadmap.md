...
| **Trạng thái CI Lõi** | ✅ **PASS 1787/1787 · SKIP 0** | `python/tests/` — 65 bộ test; `verify` quét 0 artifact ⇒ mã 1; gate chuẩn mực khoá ở `digests.lock` (job Frozen artifacts); wheel đã cài chạy cả hành trình (job `wheel-smoke`); cổng CI chặn mọi test bị skip · `tests_linux/` 26/26 trên gpio-sim + i2c-stub/lm75 + vkms (job `linux-hal`) · `security.yml`: pip-audit, gitleaks toàn lịch sử, CodeQL · extra `cloud` trên litellm thật + giấy phép Q-11 (job `cloud-extra`) |
