...
│    • Q-21 đề xuất: mô phỏng theo tầng bằng OSS đã kiểm chứng (proposal §3.2, H.4)      │
...
│    2. Kỹ thuật trưởng duyệt Q-21 và TSK-S3-15 (golden = vết ghi chuẩn mực)             │
...
| **TSK-S2-09** | Giao diện web `sim`: cảm biến ảo, trạng thái actuator | FR-TGT-06 | V3 | ⏸ **Hoãn → Sprint 5** | Không cần để đo wedge; `sim` mặc định là CLI gõ chữ (Q-15). Khi làm: Wokwi Elements chỉ là phần hiển thị (không có chốt cửa — tự vẽ một component nhỏ), nối vào `SimSession` hiện có |
...
| **TSK-S3-13** | **Tích hợp ASR/TTS qua provider cloud** cho `sim` và `linux`, kèm tùy chọn mô hình cục bộ (CR-1.0) | FR-MDL-09, FR-PER-07 | V1 | ⏸ **Hoãn → Sprint 5** | Wedge `sim` gõ chữ không cần (Q-15); làm cùng TSK-S5-06, xem §5.2. CI âm thanh `linux` dùng `snd-aloop` / PulseAudio null-sink (Q-21) |
...
| **Q-21** | Mô phỏng theo tầng bằng OSS · 🟡 **đề xuất, chờ kỹ thuật trưởng** | SimHAL · gpio-sim · snd-aloop · khung hình + digest · C biên dịch host · Espressif QEMU · bo mạch thật; không Renode, không Wokwi | Walker C (TSK-S4-02) kiểm được trên mỗi PR trước khi bo mạch về — thêm TSK-S4-07, S4-08. Proposal §3.2 |
